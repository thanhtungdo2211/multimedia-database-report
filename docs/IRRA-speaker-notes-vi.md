...
- **Slide 12 — II. Proposed Method — IRR through MLM** *(slide 12 + 13 là một cặp, và cộng lại chính là toàn bộ đường đi mô tả trong [note-MLM-head.md](note-MLM-head.md): slide 12 = **bài toán** — che từ, vì sao che từ sinh ra quan hệ ẩn; slide 13 = **module thực thi** — cross-attention + MLM head. Khi nói, nên báo trước cho khán giả là hai slide này đi liền nhau.)*
  ...
  - **Mượn gì từ BERT — mượn đúng ba thứ, đổi đúng một thứ.** Nói theo khung này thì khán giả nhớ ngay:
    - Mượn **(1) công thức masking 15% / 80–10–10** — mỗi token có 15% xác suất bị chọn; trong số bị chọn, 80% thay bằng `[MASK]`, 10% thay bằng token ngẫu nhiên, 10% giữ nguyên. *(Code: `datasets/bases.py:170-210`, hàm `_build_random_masked_tokens_and_labels` — giống hệt BERT, không sửa gì.)*
    - Mượn **(2) dạng nhãn và hàm loss** — nhãn `mlm_labels` là id token **gốc** tại các vị trí bị chọn, mọi vị trí còn lại để 0 và bị bỏ qua hoàn toàn bằng `CrossEntropyLoss(ignore_index=0)`. Tức mô hình chỉ bị chấm điểm ở đúng những chỗ đã bị che.
    - Mượn **(3) kiến trúc head** — `Linear → QuickGELU → LayerNorm → Linear(vocab_size)`, đúng khuôn MLM head của BERT (chi tiết ở slide 13).
    - Đổi đúng **một** thứ: **ngữ cảnh dùng để đoán**. BERT chỉ self-attention trong chính câu văn; IRRA cho thêm **toàn bộ patch ảnh** làm ngữ cảnh. Chỉ một thay đổi này biến một bài toán NLP thuần thành cơ chế alignment đa phương thức — và nó chính là chữ **IR** (Implicit Relation) trong tên IRRA.
  - **Che theo xác suất, không theo từ loại — chuẩn bị sẵn vì hay bị hỏi:** code không hề nhìn từ loại, điều kiện duy nhất là `0 < token < 49405` (chỉ loại padding và ba token đặc biệt `<|mask|>`, `<|startoftext|>`, `<|endoftext|>`). `a`, `is`, `and` đều có thể bị che như `bag`. Với câu *"a man is carrying a black bag and white shoes"*, việc che trúng `(bag, white)` là **một lần bốc thăm có thể xảy ra**, không phải lựa chọn có chủ đích. Fallback duy nhất: nếu cả câu không token nào trúng 15% thì ép che token ở vị trí 1.
    - **Nhưng áp lực học lại tự dồn về từ nội dung** — che `is`/`and` thì đoán lại được chỉ bằng ngữ cảnh chữ, loss tụt về gần 0 sớm → gradient ~0 → **không tạo áp lực nhìn ảnh**. Che `bag`, `black`, `white`, `around` thì chữ không đủ, loss còn cao → gradient ép model cross-attend vào ảnh. Việc "chọn đúng từ đáng học" do **hình dạng của loss** làm, không do ai lọc từ loại trước.
    - **Đơn vị che là token BPE, không phải từ** — từ dài tách thành nhiều mảnh, che một mảnh thì các mảnh còn lại vẫn hiện. Đây đúng là hạn chế cấp cụm từ ở slide 22 và là lý do hướng mở rộng đề xuất *span/phrase masking*.
    - **Mỗi epoch che một kiểu khác** — masking chạy trong `__getitem__` nên cùng một câu sẽ bị che khác nhau qua các epoch; qua nhiều epoch thì gần như mọi từ đều có lúc bị che.
  - **Vì sao 80–10–10 chứ không luôn thay `[MASK]`** — nếu luôn thay `[MASK]`, mô hình chỉ học cách xử lý riêng token `[MASK]`, mà khi test không có `[MASK]` nào → lệch train/test. Trộn thêm token ngẫu nhiên và token giữ nguyên buộc mô hình phải "đề phòng" ở **mọi** vị trí, tức phải biểu diễn tốt **mọi** token chứ không chỉ chỗ bị che. *(Code: nếu không token nào được chọn thì ép mask ít nhất một token.)*
  - **Che từ tác động lên "quan hệ ẩn" như thế nào — mạch lập luận bốn bước, đây là phần cần nói kỹ nhất:**
    - **Bước 1 — tạo ra một khoảng trống mà chữ không lấp được.** Che `white` trong `a [MASK] purse`: chỉ nhìn phần câu còn lại thì black, red, brown, blue… đều hợp ngữ pháp như nhau. Thông tin để chọn đúng **không tồn tại trong văn bản**, nó chỉ tồn tại trong ảnh.
    - **Bước 2 — ép mô hình đi tìm thông tin đó ở đúng chỗ.** Để giảm được loss, mô hình buộc phải: nhận ra từ bị che thuộc loại thuộc tính nào (màu, ở vị trí "cái túi") → cross-attend vào **đúng** vùng patch chứa cái túi, không phải nền hay quần → đọc màu từ đặc trưng thị giác vùng đó → ánh xạ ngược thành token từ vựng. Không có đường tắt nào khác để đoán đúng.
    - **Bước 3 — "ẩn" nằm ở chỗ không ai gán nhãn cho bước 2.** Không có nhãn nào nói "vùng này là cái túi", không có nhãn attention nào chỉ định từ nào ứng với patch nào. Sự tương ứng từ ↔ vùng ảnh được mô hình **tự học ra**, chỉ từ **áp lực phải đoán đúng từ**. Đó chính xác là nghĩa của *implicit* đã chốt ở slide 7.
    - **Bước 4 — hệ quả lan ngược, và đây mới là thứ thật sự có giá trị khi truy hồi.** Áp lực đó buộc **image encoder phải giữ lại thông tin không gian/cục bộ trong các patch feature** thay vì dồn hết vào `[CLS]`, và buộc **text encoder phải biểu diễn được từng thuộc tính riêng lẻ** thay vì chỉ một embedding tổng quát. Gradient lan ngược vào chính hai CLIP encoder nên nó cải thiện luôn `i_feats`/`t_feats` — tức là cải thiện đúng hai vector dùng cho SDM, ID và cho truy hồi lúc test. *(Lập luận đầy đủ ở [note-MLM-head.md](note-MLM-head.md); hệ quả "bỏ nhánh phụ mà không mất kiến thức" nói ở slide 16.)*
  - **Đây chính là khoảng trống mà SDM và ID loss không lấp được** — hai loss kia chỉ nhìn thấy **hai vector đã pool** (`[CLS]` và `[EOS]`), mọi chi tiết không gian (áo ở đâu, túi ở đâu) đã bị trung bình hóa mất trong pooling và **không có cơ chế nào ép giữ lại**. MLM là loss duy nhất trong ba loss hoạt động ở mức **token × patch**. Nói được câu này là nối trọn slide 12 ↔ slide 14 ↔ slide 15, và giải thích luôn vì sao ablation ở slide 20 cho thấy IRR và SDM bổ sung nhau chứ không trùng nhau.
  - **Ba từ bị che trong ví dụ = ba loại quan hệ khác nhau** (cho thấy MLM học nhiều thứ, không chỉ màu sắc): `shoes` — danh từ, tên vật thể → cần nhận diện đối tượng; `white` — tính từ, thuộc tính → cần đọc màu ở đúng vùng; `around` — giới từ, quan hệ không gian → cần hiểu bố cục ("quanh eo").
  - **Hàm loss** — cross-entropy chỉ tính trên vị trí bị chọn: $\mathcal L_{\mathrm{IRR}} = -\frac{1}{|\mathcal M|}\sum_{i\in\mathcal M}\log p_\theta(w_i \mid \hat T, I)$, với $\mathcal M$ là tập vị trí bị che, $\hat T$ là câu bị che, $I$ là ảnh. Chú ý điều kiện có **cả $\hat T$ và $I$** — đó là toàn bộ khác biệt so với BERT, vốn chỉ có $\hat T$.
  - **Figure 3 — cơ chế alignment, giải thích bằng ẩn dụ mỏ neo:** từ thật `bag` đóng hai vai ở hai đầu — ở **đầu vào** nó đã bị thay bằng `[MASK]` nên model không thấy; ở **đầu ra** nó là nhãn, ứng với **một hàng trong ma trận trọng số lớp cuối `mlm_head.fc`** (`nn.Linear(512, vocab_size)`, `model/build.py:52-59`). Hàng đó chính là **mỏ neo**.
    - Tính chất khiến nó làm được mỏ neo: nó **dùng chung cho mọi mẫu chứa từ `bag`** và **không phụ thuộc vào ảnh/câu nào đang ở trong batch** — một điểm đích chung cho mọi cặp nói về cái túi. Cross-entropy kéo biểu diễn hợp nhất (token bị che + patch ảnh) về phía điểm đó, nên bằng chứng thị giác và ngữ cảnh văn bản **gián tiếp** bị kéo lại gần nhau.
    - ⚠️ Nói cho đúng: hàng này **không tĩnh, không đóng băng** — nó là tham số học được, cập nhật mỗi step; và nó **không phải** token embedding table của CLIP (IRRA khởi tạo `mlm_head.fc` mới, không tie trọng số). Đừng nói "embedding cố định trong từ điển", hãy nói "một vector đích dùng chung cho từ đó, độc lập với mẫu".

- **Slide 13 — II. Proposed Method — Multimodal Interaction** *(nửa sau của cặp: slide 12 nói **vì sao phải nhìn ảnh**, slide 13 nói **nhìn bằng cơ chế nào**. Ba module dựng ở đây — `cross_attn`, `cross_modal_transformer`, `mlm_head` — chỉ được xây khi `loss_names` có `mlm`, xem `model/build.py:26-60`.)*
  - Dẫn: "Slide trước đặt ra yêu cầu: token chữ phải lấy được thông tin từ ảnh. Slide này là cơ chế thực hiện yêu cầu đó."
  - **Q, K, V — giải thích bằng ẩn dụ tra cứu, đừng bằng công thức:** **Q** (Query, "tôi đang cần tìm gì") = hidden state của token trong **câu bị che**; **K** (Key, "tôi chứa nội dung gì") = **visual token** (192 vùng ảnh, **chưa pool**); **V** (Value, "nội dung thực sự lấy về") = **visual token**. Cơ chế: mỗi Query so khớp toàn bộ Key để tính trọng số chú ý, rồi lấy tổng có trọng số của các Value — $\operatorname{MCA}(Q,K,V)=\operatorname{softmax}\!\big(\tfrac{QK^\top}{\sqrt d}\big)V$.
  - **Diễn giải một câu:** token chữ bị che chủ động **hỏi** ảnh *"vùng nào của bức ảnh liên quan đến tôi?"*, rồi mang thông tin từ vùng đó về. Chính phép hỏi này là bước 2 trong mạch lập luận ở slide 12 — nó là **cơ chế vật lý** để "quan hệ ẩn" hình thành, chứ bản thân nó không được giám sát trực tiếp.
  - **Nhấn mạnh "chưa pool"** — Key/Value là **toàn bộ patch feature**, không phải vector `[CLS]`. Nếu dùng `[CLS]` thì mọi thông tin không gian đã mất và MLM sẽ thoái hóa về đúng BERT gốc. Đây là chi tiết dễ bỏ sót nhưng là điều kiện cần để cả cơ chế hoạt động.
  ...
  - **One-way (một chiều)** — chỉ chữ hỏi ảnh, **không** có chiều ngược lại. Lý do thực dụng: nhiệm vụ cuối là **dự đoán token văn bản**, nên chỉ cần làm giàu phía văn bản; chiều ngược lại là dư thừa với mục tiêu này.
  - **Nhưng một chiều KHÔNG có nghĩa chỉ một encoder được học** — câu này nên nói thẳng vì rất dễ bị hỏi vặn: `mlm_head` chỉ xuất dự đoán ở vị trí token text, nhưng vì `image_feats` được dùng làm **Key/Value**, gradient của MLM loss chảy ngược vào **cả** `encode_image` lẫn `encode_text`, cộng thêm cập nhật cho ba module mới. MLM loss huấn luyện **cả hai** encoder gốc, chỉ là "bài kiểm tra" được đặt ở phía text.
  - **Đường đi forward đầy đủ** *(`model/build.py:125-141`)* — `mlm_feats = encode_text(mlm_ids)` (feature từng token của câu bị che, chưa pool) → `x = cross_former(mlm_feats, image_feats, image_feats)` (Q = text, K/V = toàn bộ patch ảnh) → `x = mlm_head(x)` → phân phối xác suất trên vocab tại từng vị trí. Đối chiếu sơ đồ slide 8: đây đúng là khối 3.
  - **`4 Transformer blocks`** — sau cross-attention, kết quả đi tiếp qua 4 block self-attention + feed-forward để các token văn bản (đã có thông tin ảnh) tiếp tục trao đổi với nhau, tức "hội ý" trước khi chốt từ.
  ...
  - **MLM head** — `Linear → QuickGELU → LayerNorm → Linear(vocab_size)`, cho phân phối trên toàn từ điển tại mỗi vị trí bị che. Đây đúng là khuôn head của BERT — tức "thứ thứ ba mượn từ BERT" đã nêu ở slide 12.
  - **Ba kiểu module trong Figure 4:** (a) **Co-attention** — hai nhánh Transformer song song hỏi nhau qua lại, nhiều tham số nhất (33.62M), chậm nhất (24.30 ms); (b) **Merged attention** — nối token hai modality thành chuỗi dài rồi self-attend, chuỗi dài → chi phí attention tăng bình phương; (c) **Ours — one-way** — chỉ chữ hỏi ảnh, đúng nhu cầu của MLM, rẻ nhất (6.42 ms). Số liệu so sánh ở slide 21.
